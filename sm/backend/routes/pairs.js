...
router.get('/suggestions', auth, (req, res) => {
  ...
  const mySkills = db.skills.find({ user_id: req.user.id });
  const myWantLearn = mySkills.filter(s => s.type === 'learn').map(s => s.subject);
  const myCanTeach = mySkills.filter(s => s.type === 'teach').map(s => s.subject);

  // Lấy danh sách đã ghép hoặc đã gửi/nhận yêu cầu
  const myPairs = db.pairs.all().filter(p =>
    (p.requester_id === req.user.id || p.target_id === req.user.id)
  );

  const suggestions = db.users.all()
    .filter(u => u.id !== req.user.id && !u.is_banned && !['admin', 'moderator'].includes(u.role))
    .map(u => {
      const theirSkills = db.skills.find({ user_id: u.id });
      const score = calcScore(mySkills, theirSkills, me, u);

      // Lấy tất cả cặp với người này
      ...
      // Lấy cặp đang hoạt động (pending/active) - chỉ block nếu còn active/pending
      const activePairRecord = allPairsWithUser.find(p =>
        ['pending', 'active'].includes(p.status)
      );
      // Số lần đã ghép cặp thành công trước đây
...
router.post('/request', auth, (req, res) => {
  ...
  const target = db.users.findById(target_id);
  if (!target || target.is_banned) return res.status(404).json({ error: 'Không tìm thấy người dùng' });

  // Kiểm tra đã tồn tại yêu cầu đang pending hoặc active chưa
  // Cho phép ghép lại nếu cặp trước đã completed / rejected / cancelled
  const existing = db.pairs.all().find(p =>
    ['pending', 'active'].includes(p.status) &&
    (
      (p.requester_id === req.user.id && p.target_id === target_id) ||
      (p.requester_id === target_id && p.target_id === req.user.id)
    )
  );
  if (existing) return res.status(409).json({ error: 'Bạn đang có ghép cặp chưa hoàn thành với người này', pair: existing });

  const me = db.users.findById(req.user.id);
  const pair = db.pairs.insert({
...
router.put('/:id/accept', auth, (req, res) => {
  const pair = db.pairs.findById(req.params.id);
  ...
  db.pairs.update(pair.id, { status: 'active' });
  db.users.incr(pair.requester_id, 'total_pairs');
  db.users.incr(pair.target_id, 'total_pairs');
  ...
  // Badge
  const rPairs = db.pairs.find({ requester_id: pair.requester_id, status: 'active' }).length +
    db.pairs.find({ target_id: pair.requester_id, status: 'active' }).length;
  const tPairs = db.pairs.find({ requester_id: pair.target_id, status: 'active' }).length +
    db.pairs.find({ target_id: pair.target_id, status: 'active' }).length;
  const io = req.app.get('io');
  if (rPairs >= 1) grantBadge(pair.requester_id, 'first_pair', io);
  if (tPairs >= 1) grantBadge(pair.target_id, 'first_pair', io);
  if (rPairs >= 5) grantBadge(pair.requester_id, 'five_pairs', io);
  if (tPairs >= 5) grantBadge(pair.target_id, 'five_pairs', io);
...
router.put('/:id/reject', auth, (req, res) => {
  ...
  if (pair.target_id !== req.user.id) return res.status(403).json({ error: 'Không có quyền' });
  db.pairs.update(pair.id, { status: 'rejected' });
  const me = db.users.findById(req.user.id);
  const notif = db.notifications.insert({
...
router.put('/:id/cancel', auth, (req, res) => {
  const pair = db.pairs.findById(req.params.id);
  if (!pair) return res.status(404).json({ error: 'Không tìm thấy' });
  if (pair.requester_id !== req.user.id && pair.target_id !== req.user.id)
    return res.status(403).json({ error: 'Không có quyền' });
  if (!['pending', 'active'].includes(pair.status))
    return res.status(400).json({ error: 'Không thể hủy ở trạng thái này' });

  const wasActive = pair.status === 'active';
  db.pairs.update(pair.id, { status: 'cancelled' });
  if (wasActive) {
    db.users.incr(pair.requester_id, 'total_pairs', -1);
    db.users.incr(pair.target_id, 'total_pairs', -1);
  }
  const me = db.users.findById(req.user.id);
...
// Xác nhận hoàn thành học (2 phía)
router.put('/:id/confirm-complete', auth, (req, res) => {
  const pair = db.pairs.findById(req.params.id);
  if (!pair) return res.status(404).json({ error: 'Không tìm thấy' });
  ...
  const updated = db.pairs.update(pair.id, update);

  const me = db.users.findById(req.user.id);
  const otherId = isRequester ? pair.target_id : pair.requester_id;

  // Nếu cả 2 đã xác nhận → hoàn thành
  if (updated.requester_confirmed && updated.target_confirmed) {
    db.pairs.update(pair.id, { status: 'completed', completed_at: new Date().toISOString() });
    db.users.incr(pair.requester_id, 'reputation_score', 20);
    db.users.incr(pair.target_id, 'reputation_score', 20);
    const io = req.app.get('io');
    [pair.requester_id, pair.target_id].forEach(uid => {
      const notif = db.notifications.insert({
    ...
    // Badge
    const rTotal = db.pairs.find({ status: 'completed' }).filter(p => p.requester_id === pair.requester_id || p.target_id === pair.requester_id).length;
    const tTotal = db.pairs.find({ status: 'completed' }).filter(p => p.requester_id === pair.target_id || p.target_id === pair.target_id).length;
    if (rTotal >= 10) grantBadge(pair.requester_id, 'complete_10', io);
    if (tTotal >= 10) grantBadge(pair.target_id, 'complete_10', io);
...
// Đánh giá (chỉ sau khi completed)
router.post('/:id/review', auth, (req, res) => {
  ...
  const existing = db.reviews.findOne({ pair_id: pair.id, reviewer_id: req.user.id });
  ...
  if (!knowledge || !teaching || !attitude || !punctuality)
    return res.status(400).json({ error: 'Vui lòng chấm đủ 4 tiêu chí' });
  ...
  // Cập nhật rating
  const allRevs = db.reviews.find({ reviewee_id });
  const avg = allRevs.reduce((s, r) => s + (r.knowledge + r.teaching + r.attitude + r.punctuality) / 4, 0) / allRevs.length;
  db.users.update(reviewee_id, { teaching_rating: +avg.toFixed(2) });
  db.users.incr(reviewee_id, 'reputation_score', 10);

  const io = req.app.get('io');
  // Badge
  if (avg >= 4.5) grantBadge(reviewee_id, 'top_rated', io);
  const rep = db.users.findById(reviewee_id)?.reputation_score || 0;
  if (rep >= 300) grantBadge(reviewee_id, 'rep_300', io);
  if (rep >= 1000) grantBadge(reviewee_id, 'rep_1000', io);
...
router.get('/my/all', auth, (req, res) => {
  const { status } = req.query;
  let myPairs = db.pairs.all().filter(p =>
    p.requester_id === req.user.id || p.target_id === req.user.id
  );
  if (status) myPairs = myPairs.filter(p => p.status === status);
  myPairs = myPairs.sort((a, b) => new Date(b.created_at) - new Date(a.created_at));

  const result = myPairs.map(p => {
    const partnerId = p.requester_id === req.user.id ? p.target_id : p.requester_id;
    const partner = db.users.findById(partnerId);
    const myReview = db.reviews.findOne({ pair_id: p.id, reviewer_id: req.user.id });
    const theirReview = db.reviews.findOne({ pair_id: p.id, reviewer_id: partnerId });
    return {
      ...p,
      partner: safe(partner),
      partner_skills: db.skills.find({ user_id: partnerId }),
...
// Tình nguyện viên
router.get('/volunteers/list', auth, (req, res) => {
  const { subject } = req.query;
  let vols = db.users.all().filter(u => u.id !== req.user.id && u.is_volunteer && !u.is_banned);
  if (subject) vols = vols.filter(u =>
    db.skills.find({ user_id: u.id, type: 'teach' }).some(s => s.subject.toLowerCase().includes(subject.toLowerCase()))
  );
  res.json(vols.map(u => ({
    ...safe(u),
